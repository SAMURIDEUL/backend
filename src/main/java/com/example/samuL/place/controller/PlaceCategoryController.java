...
public class PlaceCategoryController {
    ...
    // 사진 추가, 썸네일 추가
    @Operation(summary = "카테고리 id의 장소들 조회", description = "각 카테고리 id(1 - 13)의 place들을 조회합니다. lat, lon 제공 시 거리순으로 정렬되며, 미제공 시 ID순(최신순)으로 정렬됩니다.")
    ...
    public ResponseEntity<OkResponse<PlaceSelectScroll>> getPlacesCategory(
            ...
            @Parameter(description = "도/시") @RequestParam(required = false) String city,
            @Parameter(description = "구/군") @RequestParam(required = false) String district,
            @Parameter(description = "동/읍/면") @RequestParam(required = false) String subdistrict,
            @Parameter(description = "가게 이름으로 검색") @RequestParam(required = false) String keyword,
            @Parameter(description = "위도") @RequestParam(required = false) Double lat,
            @Parameter(description = "경도") @RequestParam(required = false) Double lon,
            @Parameter(description = "다음 페이지 nextCursor값/ hasNext값이 true이면 다음 페이지가 존재함으로 lastId값으로 다음 페이지 이동/ 다음페이지 없으면 null 반환") @RequestParam(required = false) BigInteger lastId,
            @Parameter(description = "한 번에 가져올 장소 개수") @RequestParam(defaultValue = "20") int size,
        ...
        PlaceSelectScroll response = placeService.getPlaceDetail(categoryId, city, district, subdistrict, keyword, lat,
                lon, lastId, size);
